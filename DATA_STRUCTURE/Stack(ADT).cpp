...
void calculator::get_formular() {
	...
	for (int i = 0; i < 100; i++) {
		if (get[i] != '\0') {
			...
			else if (is_exist_decimal) {		//소수부가 존재하는 경우
	...
	//formular_simplification();
}

void calculator::formular_simplification()			//사용못함...
{
	for (int i = 0; i < length; i++) {
		if (formular[i] == '-' - '0' && formular[i+1] >0) {
			formular[i] = '+' - '0';
			formular[i + 1] *= -1;
		}
	}
	for (int i = 0; i < length; i++) {
		if (formular[i] <= 0 && formular[i + 1 <= 0]) {
			...
			else if (formular[i] == '+' - '0' && formular[i + 1] == '+' - '0') {	//++
				formular[i + 1] = '+' - '0';
				formular_drop(i);
			}
			else if (formular[i] == '*' - '0' && formular[i + 1] == '+' - '0') {	//*+
...
void calculator::formular_drop(int i) {
